...
/**
 * Eventos da agenda cultural, do separador "EVENTOS" (SHEET_EVENTOS_URL).
 * Colunas: Categoria | Nome | Zona | Notas (Notas trazem data/hora/local).
 * Devolve o texto formatado (centro primeiro) ou null.
 */
export async function eventosDaSheet(): Promise<string | null> {
  const url = process.env.SHEET_EVENTOS_URL;
  if (!url) return null;
  let linhas: string[][] | null = null;
  try {
    const r = await fetch(url, { next: { revalidate: 600 } } as RequestInit);
  ...
  const porCategoria = new Map<string, string[]>();
  for (const l of linhas.slice(1)) {
    const categoria = (l[0] ?? '').trim() || 'Eventos';
    const nome = (l[1] ?? '').trim();
    const zona = (l[2] ?? '').trim();
    const notas = (l[3] ?? '').trim();
    if (!nome) continue;
  ...
  const seccoes = Array.from(porCategoria.entries())
    ...
    .join('\n\n');
  return (
    '# AGENDA CULTURAL DE BRAGA — eventos oficiais\n' +
    'Recomenda APENAS estes eventos. As notas trazem data, hora e local exatos — usa-os. Se perguntarem por um dia específico, filtra pelos que ocorrem nesse dia. Dá prioridade aos marcados como centro. Nunca inventes eventos, datas nem locais. Se a agenda parecer de outro mês, avisa que podem confirmar em visitbraga.travel.\n\n' +
    seccoes +
    '\n'
  );
}
